...
# # Checkpoint configuration for dual loading, note for testing only
# checkpoint_loading_config = dict(
#     # module_checkpoint='/dataset/xxxx/mmdet3d/mmdet3d_output_model/UniBEV/train/TemFeatPred_nus_LC_cnw_256_modality_dropout/epoch_6.pth',
#     module_prefixes=['history_pts_feat_predictor.', 'history_img_feat_predictor.']
# )

dataset_type = 'NuScenesDataset'
# data_root = 'data/nuscenes/'
# sub_dir = 'mmdet3d_old_cor/'
data_root = '/dataset/xxxx/mmdet3d/data/nuscenes/'
sub_dir = ''
train_ann_file = sub_dir + 'nuscenes_infos_train.pkl'
# train_ann_file = sub_dir + 'nuscenes_infos_train_debug.pkl'
val_ann_file = sub_dir + 'nuscenes_infos_val.pkl'
work_dir = 'outputs/train/unibev_featpred-CrMdFusionUncert-Stage2_nus_LC_cnw_256_modality_dropout'

load_from = 'outputs/train/unibev_featpred-CrMdFusionUncert_nus_LC_cnw_256_modality_dropout/epoch_12.pth'
...
data = dict(
    ...
    shuffle=False, # for always shuffle
    train=dict(
            type=dataset_type,
            data_root=data_root,
    ...
    test=dict(
        ...
        test_mode=True,
        box_type_3d='LiDAR'),
    shuffler_sampler=dict(type='DistributedGroupSampler', shuffle=False),
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        as_two_stage=False,
        transformer=dict(
            type='UniBEVTransformer_HistoryFeatPredictor_CrMdFusion_Uncertainty',
            ...
            adj_num_frames = adj_pred_num,
            pts_feat_pred = True,
            img_feat_pred = True,
            history_decoder=dict(
                type='LongTermPredictorResUncertainty',
                in_channels=256,
                out_channels=256,
                embed_dims=128,
                num_history=adj_pred_num,
                reduction=feat_reduction,
                bev_h=200, 
                bev_w=200,
                decoder=dict(
                    ...
                    transformerlayers=dict(
                        ...
                        attn_cfgs=[
                            dict(
                                ...
                                embed_dims=128//feat_reduction,
                                num_heads=2,
                                num_levels=1,
                                num_bev_queue=adj_pred_num,
                                dropout=0.0)
                        ],
                        ffn_cfgs=dict(
                            type='FFN',
...
log_level = 'INFO'
custom_hooks = [
    dict(type='FreezeParametersHook',
         keep_patterns=['cross_modal_module', 
                        'img_pred_weights', 'img_cur_weights',
                        'pts_pred_weights', 'pts_cur_weights',
                        'history_img_feat_predictor',
                        'history_pts_feat_predictor',
                        ],
         freeze_BN=True,
         priority=70),
