...
import numpy as np
import mmdet3d
from .dist_utils import decide_global_skip


@DETECTORS.register_module()
class UniBEV_NewMD(MVXTwoStageDetector):
    """
    UniBEV model:
        ...
        todo
        which allows a new modality missing strategy.
    ...
    def __init__(self,
        ...
        self.both_miss_prob = both_miss_prob
    ...
    def extract_img_feat(self, img, img_metas=None):
        ...
        if img is not None:
            ...
            ## mannually add checkpoint
            from torch.utils.checkpoint import checkpoint
            def backbone_forward(x):
                return self.img_backbone(x)
            if self.training:
                img_feats = checkpoint(backbone_forward, img)
            else:
                img_feats = self.img_backbone(img)
    ...
    def extract_feat(self, img, points, radar_points, img_metas=None):
        ...
        # print("Mem before backbone:", torch.cuda.memory_allocated() / 1024**2, "MB")  # or voxel encoder
        img_feats = self.extract_img_feat(img, img_metas) if self.use_camera else None
        # print("Mem after backbone:", torch.cuda.memory_allocated() / 1024**2, "MB")
    ...
    def forward_train(self,
        ...
        device = img.device if img is not None else points.device

        # assert img_metas.__len__() == 1, 'Only support batch size 1 for now.'
        # if img_metas[0].get('valid_modal_flag', False):
        #     flag_lc = img_metas[0]['valid_modal_flag']
        #     self.use_lidar = flag_lc['lidar'] == 1 # drop lidar based on the flag
        #     if not self.use_lidar: 
        #         points = None
        #     else:
        #         assert points is not None
        #     self.use_camera = flag_lc['camera'] == 1 # drop camera based on the flag
        #     if not self.use_camera:
        #         img = None
        #     else:
        #         assert img is not None
        # print(f"flag_l: {self.use_lidar}, flag_c: {self.use_camera}, flag_r: {self.use_radar}")

        # if (not self.use_camera) and (not self.use_lidar) and (not self.use_radar):
        #     # print('Both Lidar and Camera are missing, skipping forward pass.')
        #     zero_tensor = lambda: torch.tensor([0.0], device=device, dtype=torch.float32, requires_grad=True)
        #     zero_losses = {
        #         'loss_cls': zero_tensor(),
        #         'loss_bbox': zero_tensor(),
        #         'd0.loss_cls': zero_tensor(),
        #         'd0.loss_bbox': zero_tensor(),
        #         'd1.loss_cls': zero_tensor(),
        #         'd1.loss_bbox': zero_tensor(),
        #         'd2.loss_cls': zero_tensor(),
        #         'd2.loss_bbox': zero_tensor(),
        #         'd3.loss_cls': zero_tensor(),
        #         'd3.loss_bbox': zero_tensor(),
        #         'd4.loss_cls': zero_tensor(),
        #         'd4.loss_bbox': zero_tensor(),
        #     }
        #     zero_losses['loss'] = sum([v.clone() for v in zero_losses.values()])
        #     return zero_losses
        #     # return dict(loss=torch.tensor([0.0], device=device, dtype=torch.float32, requires_grad=True))            
        if decide_global_skip(p=self.both_miss_prob, device=device):
            # 返回一个 requires_grad=True 的 scalar tensor，确保 backward 不出错
            print('Skip this batch')
            zero_loss = torch.tensor([0.0], device=device, requires_grad=True)
            return dict(loss=zero_loss)
