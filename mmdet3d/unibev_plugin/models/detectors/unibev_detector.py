...
class UniBEV(MVXTwoStageDetector):
    ...
    def forward_test(self, img_metas, img=None, points=None, radar=None, **kwargs):
        ...
        print('img is None: {}'.format(img is None), 'points is None: {}'.format(points is None))
                
        img = [img] if img is None else img
        points = [points] if points is None else points
        radar = [radar] if radar is None else radar

        for var, name in [(img, 'img'), (points, 'points'), (radar, 'radar'), (img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
    ...
    def simple_test(self, points, img_metas, img=None, radar = None, rescale=False):
        ...
        # adapt to random modality dropout in test time
        if img is None:
        ...
        if points is None:
            ...
            self.use_lidar = True
        assert self.use_radar == False and radar is None, 'Radar modality is not supported in UniBEV_FeatPred.'
        if self.use_camera == False and self.use_lidar == False:
            from mmdet3d.core.bbox import LiDARInstance3DBoxes
            empty_bbox_list = [{
                'pts_bbox': {
                    'boxes_3d': LiDARInstance3DBoxes(torch.empty(0, 9)),  # 0个框，每个框9个参数
                    'scores_3d': torch.empty(0),  # 0个置信度分数
                    'labels_3d': torch.empty(0, dtype=torch.long)  # 0个类别标签
                }
            }]
            return empty_bbox_list, None
