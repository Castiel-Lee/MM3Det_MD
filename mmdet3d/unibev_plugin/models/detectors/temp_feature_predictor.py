...
class TempFeaturePredictor(MVXTwoStageDetector):
    """
    TemFeatPred model:
    A model that predicts temporal features from LiDAR, Camera, and Radar data.
    - radar data is not supported yet
    Args:
    """
    ...
    def forward_train(self,
        ...
        img_feats, lidar_feats, radar_feats = None, None, None
        losses = dict()
        ...
        if outs.get('predict_recon_loss', False):
            losses.update(outs['predict_recon_loss'])
            outs.pop('predict_recon_loss')
    ...
    def simple_test(self, points, img_metas, img=None, radar = None, rescale=False):
        raise NotImplementedError( 'Not implemented simple_test function yet')
        img_feats, lidar_feats, radar_feats = self.extract_feat(img=img, points=points, radar_points=radar, img_metas=img_metas)
