...
# from .utils.feature_loader import UniBevFeatureLoader
from .utils.feature_loader_cache import UniBevFeatureLoader
# from .utils.feature_loader_cachecpu import UniBevFeatureLoader
...
class UniBEVTransformer_HistoryFeatPredictor_TrainerOnly(BaseModule):
    ...
    def __init__(self,
                 ...
                 adj_num_frames = 6,
                 **kwargs):
        super(UniBEVTransformer_HistoryFeatPredictor_TrainerOnly, self).__init__(**kwargs)
        ...
        self.feat_loader = UniBevFeatureLoader(feat_load_path)
        parent_path = osp.dirname(feat_load_path.rstrip('/'))
        self.feat_loader.load_frame_metadata(osp.join(parent_path, info_file))
    ...
    def process_predict_pts_img_feats(self, batch_frame_names):
        loader_result_pts = self.feat_loader.load_batch_features_with_history(
            frame_names=batch_frame_names,
            feature_keys=['ori_pts_bev_embed'],
            max_history=self.adj_num_frames
        )
        historical_feats_pts, current_feat_pts = self.feat_loader.prepare_temporal_data(
            loader_result_pts, 
            feature_key='ori_pts_bev_embed',
            bev_h=self.bev_h, 
            bev_w=self.bev_w,
        )

        results_pts = self.history_pts_feat_predictor(
            historical_feats=historical_feats_pts,  # List[Tensor(B,256,200,200)]
            current_feat=current_feat_pts,          # Tensor(B,256,200,200)
            return_loss=True
        )

        loader_result_img = self.feat_loader.load_batch_features_with_history(
            frame_names=batch_frame_names,
            feature_keys=['ori_img_bev_embed'],
            max_history=self.adj_num_frames
        )
        historical_feats_img, current_feat_img = self.feat_loader.prepare_temporal_data(
            loader_result_img, 
            feature_key='ori_img_bev_embed',
            bev_h=self.bev_h, 
            bev_w=self.bev_w,
        )
        results_img = self.history_img_feat_predictor(
            historical_feats=historical_feats_img,  # List[Tensor(B,256,200,200)]
            current_feat=current_feat_img,          # Tensor(B,256,200,200)
            return_loss=True
        )

        return results_pts, results_img 
    ...
    def forward(self,
        ...
        assert self.training is True, 'This module is only for training'
        ...
        # else:
        #     pts_bev_embed = None
        img_bev_embed, pts_bev_embed = None, None
        ...
        # print('batch_frame_names:', batch_frame_names)
    
        results_pts, results_img = self.process_predict_pts_img_feats(batch_frame_names)

        predict_recon_loss = {
            'loss_feat_pred_pts': results_pts['loss_temporal'],
            'loss_feat_pred_img': results_img['loss_temporal'],
        }

        img_bev_embed = results_img['pred_current_flat']
        pts_bev_embed = results_pts['pred_current_flat']
...
@TRANSFORMER.register_module()
class UniBEVTransformer_CrossMDHistoryFeatPredictor_TrainerOnly(BaseModule):
    """Implements the UniBEV transformer, based on Detr3D transformer.
    Args:
        as_two_stage (bool): Generate query from encoder features.
            Default: False.
    ...
    def __init__(self,
                 ...
                 img_feat_pred = True,
                 **kwargs):
        super(UniBEVTransformer_CrossMDHistoryFeatPredictor_TrainerOnly, self).__init__(**kwargs)
        ...
        history_decoder.query_dim = copy.deepcopy(history_decoder.in_channels)
        history_decoder.in_channels = 2 * history_decoder.in_channels
        self.pts_feat_pred = pts_feat_pred
        self.img_feat_pred = img_feat_pred
        if self.pts_feat_pred:
            self.history_fused_feat_predictor_pts = build_transformer_layer_sequence(history_decoder)
        if self.img_feat_pred:
            self.history_fused_feat_predictor_img = build_transformer_layer_sequence(history_decoder)
    ...
    def get_history_features(self, batch_frame_names, modaility):
        loader_result = self.feat_loader.load_batch_features_with_history(
            frame_names=batch_frame_names,
            feature_keys=['ori_pts_bev_embed'] if modaility == 'pts' else ['ori_img_bev_embed'],
            max_history=self.adj_num_frames
        )
        historical_feats, current_feat = self.feat_loader.prepare_temporal_data(
            loader_result, 
            feature_key='ori_pts_bev_embed' if modaility == 'pts' else 'ori_img_bev_embed',
            bev_h=self.bev_h, 
            bev_w=self.bev_w,
        )
        return historical_feats, current_feat
    
    def process_feat_prediction(self, filename, feature_predictor, feature_predictor_query, feature_tuple, diff_feature_tuple, modality, B=1): 
        # history_fused_feat_predictor = self.history_fused_feat_predictor_pts \
        #     if modality == 'pts' else self.history_fused_feat_predictor_img
        history_fused_feat_predictor = feature_predictor_query 
    
        history_feature_list, current_feature = feature_tuple
        diff_hist_feats_list, current_feature_diffMD = diff_feature_tuple
            
        historical_feats, fused_historical_feats = history_feature_list, []
        for i in range(len(history_feature_list)):
            assert diff_hist_feats_list != []
            diff_hist_feats = diff_hist_feats_list[i] # (B, 256, 200, 200)
            fused_hist_feats = torch.cat((historical_feats[i], diff_hist_feats), dim=1)
            fused_historical_feats.append(fused_hist_feats)
            
        current_feat = current_feature
        current_feature_diffMD= current_feature_diffMD
    
        query_diffMD_res = history_fused_feat_predictor(
            historical_feats = fused_historical_feats,
            query_cross_modal = current_feature_diffMD,
            current_feat=current_feat,
            return_loss=True
        )
        
        query_diffMD, loss_temporal_query = query_diffMD_res['pred_current'], query_diffMD_res['loss_temporal']
        
        results = feature_predictor(
            historical_feats=historical_feats,  # List[Tensor(B,256,200,200)]
            query_cross_modal = query_diffMD,  # Tensor(B,256,200,200)
            current_feat=current_feat,          # Tensor(B,256,200,200)
            return_loss=True
        )
        results['loss_temporal'] += loss_temporal_query
        return results 
    ...
    def forward(self,
        ...
        batch_frame_names = [osp.split(meta['pts_filename'])[-1].split('.')[0]  for meta in kwargs['img_metas']]
        feature_tuple_pts = self.get_history_features(batch_frame_names, 'pts')
        feature_tuple_img = self.get_history_features(batch_frame_names, 'img')
        # print(batch_frame_names)
        ...
        if self.training:
            ...
            if self.pts_feat_pred:
                results_pts = self.process_feat_prediction(
                    ...
                    feature_predictor_query=self.history_fused_feat_predictor_pts, 
                    feature_tuple=feature_tuple_pts, 
                    diff_feature_tuple=feature_tuple_img,
                    modality='pts'
                )
            ...
            if self.img_feat_pred:
                results_img = self.process_feat_prediction(
                    ...
                    feature_predictor_query=self.history_fused_feat_predictor_img, 
                    feature_tuple=feature_tuple_img, 
                    diff_feature_tuple=feature_tuple_pts,
                    modality='img'
                )
