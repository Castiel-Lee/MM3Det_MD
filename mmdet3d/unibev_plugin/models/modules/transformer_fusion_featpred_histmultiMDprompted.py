...
class UniBEVTransformer_HistoryFeatPredictor_MultiMDHistPrompted(BaseModule):
    ...
    def __init__(self,
                 ...
                 img_feat_pred = True,
                 **kwargs):
        super(UniBEVTransformer_HistoryFeatPredictor_MultiMDHistPrompted, self).__init__(**kwargs)
        ...
        if self.pts_feat_pred:
            ...
            self.pts_feat_memory_reader = UniBevFeatureMemoryReader(verbose=False)
        if self.img_feat_pred:
            self.history_img_feat_predictor = build_transformer_layer_sequence(history_decoder)
            self.img_feat_memory_reader = UniBevFeatureMemoryReader(verbose=False)
    ...
    def process_feat_prediction(self, filename, timestamp, feat_memory_reader, feature_predictor, current_feature=None, current_feature_diffMD=None, diff_hist_tuple=None, B=1): 
        pts_pred_flag = feat_memory_reader == self.pts_feat_memory_reader
        # print('pts_pred_flag', pts_pred_flag)
        
        history_fused_feat_predictor = self.history_fused_feat_predictor_pts \
            if pts_pred_flag else self.history_fused_feat_predictor_img
        # diff_feat_memory_reader = self.img_feat_memory_reader if pts_pred_flag else self.pts_feat_memory_reader
      
        if self.training:
            assert current_feature is not None
            assert current_feature_diffMD is not None
            ...
            if history_feature_list.__len__() == 0:
                ...
                return {'loss_temporal': torch.zeros(1, device='cuda', requires_grad=True),  'pred_current_flat': None}
            # print(history_timestamp_list)
            # for i in range(len(history_feature_list)):
            #     print(history_feature_list[i][0,:7,0]) 
            
            # diff_hist_feats_list, diff_hist_filename_list, diff_hist_timestamp_list = diff_feat_memory_reader.get_history_features(timestamp, self.adj_num_frames)     
            diff_hist_feats_list, diff_hist_filename_list, diff_hist_timestamp_list = diff_hist_tuple  
               
            # print(f'*****\nhistory timestamp list: {history_timestamp_list}, \ndiff history timestamp list: {diff_hist_timestamp_list}')
            # print(f'*****\nhistory feature list:{history_feature_list[0][0,:10,0]}, \ndiff history feature list: {diff_hist_feats_list[0][0,:10,0]}')
            ...
            for i in range(len(history_feature_list)):
                ...
                if diff_hist_feats_list != []:
                    diff_hist_feats = diff_hist_feats_list[i] # (B, 40000, 256)
                    diff_hist_feats = diff_hist_feats.view(B, self.bev_h, self.bev_w, -1).permute(0, 3, 1, 2)
                    fused_hist_feats = torch.cat((hist_feat, diff_hist_feats), dim=1)
                else:
                    print(f'No valid diff history features, just use the original history features')
                    fused_hist_feats = torch.cat((hist_feat, hist_feat), dim=1)
            ...
            current_feature_diffMD = current_feature_diffMD.view(B, self.bev_h, self.bev_w, -1).permute(0, 3, 1, 2)
            ...
            if current_feature is None:
                ...
                if history_feature_list.__len__() ==0:
                    ...
                    print('no valid history and no current_feature, just give up and wait for the next valid frame')
                    return {'pred_current_flat': None}
                ...
                query_diffMD = history_fused_feat_predictor(
                    historical_feats = fused_historical_feats,
                    query_cross_modal = current_feature_diffMD,
                    return_loss=False
                )['pred_current']
    ...
    def forward(self,
        ...
        # print('Is pts_mlvl_feats None:', pts_mlvl_feats is None, 'Is img_mlvl_feats None:', img_mlvl_feats is None)
        ...
        pts_hist_tuple = self.pts_feat_memory_reader.get_history_features(batch_time_stamps[0], self.adj_num_frames)
        img_hist_tuple = self.img_feat_memory_reader.get_history_features(batch_time_stamps[0], self.adj_num_frames)
        if self.training:
            predict_recon_loss = {}
            if self.pts_feat_pred:
                ...
                results_pts = self.process_feat_prediction(
                    ...
                    current_feature_diffMD = img_bev_embed,
                    diff_hist_tuple = img_hist_tuple,
                )
            ...
            if self.img_feat_pred:
                ...
                results_img = self.process_feat_prediction(
                    ...
                    current_feature_diffMD = pts_bev_embed,
                    diff_hist_tuple = pts_hist_tuple,
                )
            ...
            if self.pts_feat_pred:
                if self.l_flag == 0: # no pts_feat, predict feat an update memory
                    results_pts = self.process_feat_prediction(
                        filename=batch_frame_names[0], 
                        timestamp=batch_time_stamps[0], 
                        feat_memory_reader=self.pts_feat_memory_reader, 
                        ...
                        current_feature_diffMD=img_bev_embed,  # use img_bev_embed as the diffMD
                        diff_hist_tuple=img_hist_tuple,  # use img_hist_tuple as the diffMD
                    )
                    if results_pts['pred_current_flat'] is not None:
                        pts_bev_embed = results_pts['pred_current_flat']
                        # print('predict pts_bev_embed:', pts_bev_embed.shape)
                        self.l_flag = 1
                else: # with pts_feat, update memory only by pts_feat
                    self.process_feat_prediction(
                        filename=batch_frame_names[0], 
                        timestamp=batch_time_stamps[0], 
                        feat_memory_reader=self.pts_feat_memory_reader, 
                        feature_predictor=self.history_pts_feat_predictor, 
            ...
            if self.img_feat_pred:
                if self.c_flag == 0: # no img_feat, predict feat an update memory
                    results_img = self.process_feat_prediction(
                        filename=batch_frame_names[0], 
                        timestamp=batch_time_stamps[0], 
                        feat_memory_reader=self.img_feat_memory_reader, 
                        feature_predictor=self.history_img_feat_predictor, 
                        current_feature=None,
                        current_feature_diffMD=pts_bev_embed,  # use pts_bev_embed as the diffMD
                        diff_hist_tuple=pts_hist_tuple,  # use pts_hist_tuple as the diffMD
                    )
                    if results_img['pred_current_flat'] is not None:
                        img_bev_embed = results_img['pred_current_flat']
                        # print('predict img_bev_embed:', img_bev_embed.shape)
                        self.c_flag = 1
                else: # with img_feat, update memory only by img_feat
                    self.process_feat_prediction(
                        filename=batch_frame_names[0], 
                        timestamp=batch_time_stamps[0], 
                        feat_memory_reader=self.img_feat_memory_reader, 
        ...
        # self.fused_feat_memory_reader.update_memory(batch_frame_names[0], batch_time_stamps[0], fused_bev_embed)
