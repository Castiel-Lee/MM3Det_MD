...
class UniBEVTransformer_HistoryFeatPredictor_FusedHistPrompted(BaseModule):
    ...
    def __init__(self,
                 ...
                 img_feat_pred = True,
                 **kwargs):
        super(UniBEVTransformer_HistoryFeatPredictor_FusedHistPrompted, self).__init__(**kwargs)
        ...
        self.fused_feat_memory_reader = UniBevFeatureMemoryReader()
    ...
    def process_feat_prediction(self, filename, timestamp, feat_memory_reader, feature_predictor, current_feature=None, current_feature_diffMD=None, B=1):
        assert current_feature_diffMD is not None 
        self.history_fused_feat_predictor = self.history_fused_feat_predictor_pts \
            if feat_memory_reader == self.pts_feat_memory_reader \
            else self.history_fused_feat_predictor_img
        if self.training:
            ...
            fused_hist_feats_list, _, _ = self.fused_feat_memory_reader.get_history_features(timestamp, self.adj_num_frames)     
            ...
            for i in range(len(history_feature_list)):
                ...
                fused_hist_feats = fused_hist_feats_list[i] # (B, 40000, 256)
                fused_hist_feats = fused_hist_feats.view(B, self.bev_h, self.bev_w, -1).permute(0, 3, 1, 2)
            ...
            query_diffMD = self.history_fused_feat_predictor(
                historical_feats = fused_historical_feats,
