...
@PIPELINES.register_module()
class RandomDropSampleModality(object):
    """Randomly drop sample during training with a given probability.

    Args:
        drop_prob (float): The probability of dropping a sample.
        Attention: Given DDP, prob of no modality is excluded in this sampling
    """
    def __init__(self, drop_prob_dict):
        self.drop_prob_dict = drop_prob_dict
        assert isinstance(drop_prob_dict, dict), \
            f'drop_prob_dict should be a dict, but got {type(drop_prob_dict)}'

    def __call__(self, results):
        results['valid_modal_flag'] = {}
        for key, drop_prob in self.drop_prob_dict.items():
            assert isinstance(drop_prob, float), \
                f'drop_prob should be a float, but got {type(drop_prob)}'
            # results['valid_modal_flag'][key] = 1 - (random.random() < drop_prob)
        drop_c = self.drop_prob_dict['camera']
        drop_l = self.drop_prob_dict['lidar']
        v_flag = random.random() < ((1 - drop_c)*drop_l+(1 - drop_l)*drop_c)/(1-drop_c*drop_l)
        if v_flag:
            dominator = (1 - drop_c)*drop_l+(1 - drop_l)*drop_c
            results['valid_modal_flag']['camera'] = int(random.random() < (1 - drop_c)*drop_l/dominator)
            results['valid_modal_flag']['lidar'] = 1 - results['valid_modal_flag']['camera']
            results['valid_modal_flag']['both_miss_prob'] = drop_c * drop_l
        return results
